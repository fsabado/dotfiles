...
function main()
   ...
   local t = {}
   if verbose then
      t[#t+1] = {"Node name","Calls","Average per call","Total time","%Time"}
   else
      t[#t+1] = {"Node name","Total time"}
   end

   for k, v in pairs(sorted) do
      if v["info"]["func"] ~= "(null)" then
         local average = tostring(v["info"]["total"] / v["info"]["calls"])
         local percent = tostring(100 * v["info"]["total"] / global_t)
         if verbose then
            t[#t+1] = {v["info"]["func"], tostring(v["info"]["calls"]), average, tostring(v["info"]["total"]), percent}
         else
            t[#t+1] = {v["info"]["func"], tostring(v["info"]["total"])}
         end
      end
   end
